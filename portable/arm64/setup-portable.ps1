# ============================================================================
# setup-portable.ps1 - AiTao Portable Setup for Windows
#
# Downloads and assembles a fully portable AiTao environment:
#   - Python 3.13 (embedded, no system install)
#   - Meilisearch (standalone binary)
#   - Ollama (standalone binary)
#   - All Python dependencies (pre-installed in local venv)
#
# Usage: .\setup-portable.ps1
# Target: Windows 11 ARM64 (also works on x64 via emulation)
# ============================================================================
...
# Download URLs
$PYTHON_URL     = "https://www.python.org/ftp/python/$PYTHON_VERSION/python-$PYTHON_VERSION-embed-$ARCH.zip"
$MEILI_URL      = "https://github.com/meilisearch/meilisearch/releases/download/v$MEILI_VERSION/meilisearch-windows-amd64.exe"
$OLLAMA_URL     = "https://github.com/ollama/ollama/releases/download/v$OLLAMA_VERSION/ollama-windows-$ARCH.zip"
...
Write-Host "   AiTao Portable Setup - Windows $ARCH"       -ForegroundColor Cyan
Write-Host "===============================================" -ForegroundColor Cyan
Write-Host ""
Write-Host "Versions:"
Write-Host "  Python:      $PYTHON_VERSION ($ARCH)"
Write-Host "  Meilisearch: $MEILI_VERSION (amd64, emulated on ARM)"
Write-Host "  Ollama:      $OLLAMA_VERSION ($ARCH)"
Write-Host ""
...
# ============================================================================
Write-Step "2/7" "Setting up Python $PYTHON_VERSION ($ARCH)..."
...
Write-Host "  Installing sentence-transformers, FastAPI, etc...."
Write-Host ""
& $pythonExe -m pip install --no-warn-script-location `
    --trusted-host pypi.org `
    --trusted-host files.pythonhosted.org `
...
Write-Host "  python/       Python $PYTHON_VERSION embedded"
Write-Host "  meilisearch/  Meilisearch $MEILI_VERSION"
Write-Host "  ollama/       Ollama $OLLAMA_VERSION"
